...
# file_name = "Reynolds_input_clusters.p"
# GEnx_OD, GEnx_OD_true, All_Reynolds = pickle.load(open("Clusters/" + file_name, "rb"))
...
def objFOD(X):
    ...
    for i in range(len(GEnx_OD)):
        gspdll.InitializeModel()
        Fan_c_fm, Fan_c_fpr, Fan_c_fe = scale_maps_reynolds("C", Re2_DP, Re2[i], "1_LPC_core", X[:6])
        Fan_d_fm, Fan_d_fpr, Fan_d_fe = scale_maps_reynolds("C", Re2_DP, Re2[i], "2_LPC_bypass", X[6:12])
        HPC_fm, HPC_fpr, HPC_fe = scale_maps_reynolds("C", Re25_DP, Re25[i], "3_HPC", X[12:18])
